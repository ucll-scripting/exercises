...
authors = list(set(random_name() for _ in range(200)))
authors.sort()
...
def random_commit():
    ...
    message = f'Closed #{random.randint(1, 10000)}'
    return Commit(hash, author, time, message)

def random_commits(n):
    return sorted([ random_commit() for _ in range(n) ], key=lambda x: x.time)


with open('input.txt', 'w') as out:
    out.write("\n\n".join(map(str, random_commits(10000))))